...
INTRINSIC_WEB_PID=""
cleanup() {
  if [[ -n "${INTRINSIC_WEB_PID}" ]]; then kill "${INTRINSIC_WEB_PID}" 2>/dev/null || true; fi
  ...
  wait "${INTRINSIC_WEB_PID}" 2>/dev/null || true
...
"${PREFIX}/lib/xgc_camera_calibration/intrinsic_calibrator_web.py" \
  ...
  >"${RUNTIME}/intrinsic_web.log" 2>&1 &
INTRINSIC_WEB_PID="$!"
wait_ready() {
  local port="$1" pid="$2" ready=false
  for _ in $(seq 1 150); do
    if python3 -c "import json, sys, urllib.request; p=json.load(urllib.request.urlopen('http://127.0.0.1:${port}/healthz', timeout=1)); sys.exit(0 if p['status'] == 'ok' and p['image_ready'] else 1)" >/dev/null 2>&1; then
      ready=true
      break
    fi
...
if ! wait_ready 18765 "${WEB_PID}" || ! wait_ready 18766 "${INTRINSIC_WEB_PID}"; then
  echo "Installed calibration WebUI did not become ready" >&2
  for log_file in web intrinsic_web driver roscore; do
    echo "--- ${log_file}.log ---" >&2
...
python3 -c 'import json, urllib.request; payload=json.load(urllib.request.urlopen("http://127.0.0.1:18766/healthz", timeout=2)); assert payload["status"] == "ok" and payload["image_ready"] and payload["camera_control"] is False'
python3 -c 'import urllib.request; payload=urllib.request.urlopen("http://127.0.0.1:18766/", timeout=2).read(); assert b"Camera intrinsic calibration" in payload'
